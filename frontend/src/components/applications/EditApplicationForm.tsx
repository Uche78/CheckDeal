...
import { 
  getApplicationWithBorrowersById, 
  updateApplication 
} from '../../lib/supabase/applications';
import { updateBorrower } from '../../lib/supabase/borrowers';
...
export default function EditApplicationForm({ applicationId }: Props) {
  ...
  async function loadApplication() {
    try {
      ...
      const [province, postalCode] = provincePostal.split(' ').filter(Boolean);

      // Get primary borrower
      const primaryBorrower = data.borrowers?.find(b => b.borrower_type === 'primary');
      const [firstName, ...lastNameParts] = (primaryBorrower?.full_name || '').split(' ');
      ...
      setFormData({
        ...
        dateOfBirth: primaryBorrower?.date_of_birth || '',
        employmentStatus: primaryBorrower?.employment_status || 'full-time',
        employer: primaryBorrower?.employer || '',
        annualIncome: primaryBorrower?.annual_income || 0,
        propertyAddress: street,
        city: city,
        province: province || 'ON',
        postalCode: postalCode || '',
        propertyType: data.property_type,
        purchasePrice: data.property_value,
        downPayment: data.down_payment,
        loanAmount: data.loan_amount,
        loanPurpose: data.loan_purpose || 'purchase',
      });
    } catch (err) {
  ...
  const handleSubmit = async () => {
    if (!validateStep(currentStep) || !application) {
      return;
    }
    ...
    try {
      ...
      if (primaryBorrower) {
        await updateBorrower(primaryBorrower.id, {
          full_name: fullName,
    ...
    return (
      <div className="card bg-red-50 border-red-200">
        <div className="card-body">
          ...
          <a href="/applications" className="mt-4 btn btn-primary">
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="card">
        <div className="card-body">
          ...
          {currentStep === 1 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Same fields as CreateApplicationForm Step 1 */}
                <div>
          ...
          {/* Step 2: Property Information - Same as CreateApplicationForm */}
          {currentStep === 2 && (
            <div>
          ...
          {/* Step 3: Review - Same as CreateApplicationForm */}
          {currentStep === 3 && (
            <div>
